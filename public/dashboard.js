    ...
    try {
        ...
        if (data.updateAvailable) {
            document.getElementById('updateMessage').innerHTML = 
                `Доступна новая версия: <b style="color:var(--success)">${data.latestVersion}</b> (Ваша: ${data.currentVersion})<br><br>
                 <span style="font-size:13px;">Процесс закроет сервер, скачает новые файлы и перезапустится автоматически.</span>`;
            document.getElementById('startUpdateBtn').style.display = 'flex';
        } else {
            document.getElementById('updateMessage').innerHTML = 
                `У вас установлена самая последняя версия (${data.currentVersion}).<br>Вы великолепны! ✨`;
        }
    } catch (err) {
...
async function startUpdate() {
    ...
    btn.innerHTML = `<svg class="spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite;"><line x1="12" y1="2" x2="12" y2="6"></line><line x1="12" y1="18" x2="12" y2="22"></line><line x1="4.93" y1="4.93" x2="7.76" y2="7.76"></line><line x1="16.24" y1="16.24" x2="19.07" y2="19.07"></line><line x1="2" y1="12" x2="6" y2="12"></line><line x1="18" y1="12" x2="22" y2="12"></line><line x1="4.93" y1="19.07" x2="7.76" y2="16.24"></line><line x1="16.24" y1="7.76" x2="19.07" y2="4.93"></line></svg> Обновление...`;
    btn.disabled = true;

    try {
        await fetch('/api/start-update', { method: 'POST' });
        
        document.getElementById('updateMessage').innerHTML = 
            `<b style="color:var(--success)">Обновление запущено!</b><br><br>Окно консоли может мигнуть. Подождите 10-15 секунд и обновите страницу.`;
        btn.style.display = 'none';
        
        // Автоматически пытаемся обновить страницу через 12 секунд
        setTimeout(() => {
            location.reload();
        }, 12000);

    } catch (err) {
        document.getElementById('updateMessage').innerHTML = '<span style="color:var(--error)">Не удалось запустить процесс обновления.</span>';
        btn.style.display = 'none';
    }
}